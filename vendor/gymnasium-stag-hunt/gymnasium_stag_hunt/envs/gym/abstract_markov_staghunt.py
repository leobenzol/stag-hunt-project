...
from gymnasium_stag_hunt.src.utils import print_matrix


class AbstractMarkovStagHuntEnv(Env, ABC):
    metadata = {"render_modes": ["human", "array"], "render_fps": 30, "obs.types": ["image", "coords"]}

    def __init__(self, grid_size=(5, 5), obs_type="image", enable_multiagent=True):
        """
    ...
    def render(self, mode="human", obs=None):
        """
        :param obs: observation data (passed for coord observations so we do not have to run the function twice)
            ...
            if self.obs_type == "image":
                ...
                if self.game.RENDERER:
                    ...
                    if obs is not None:
                        ...
                        print_matrix(
                            self.game.get_observation()[0] if self.enable_multiagent else self.game.get_observation(),
                            self.game_title,
