...
def place_entity_in_unoccupied_cell(used_coordinates, grid_dims, rng):
    """
    ...
    :param rng: the game's numpy ``Generator``, so placement is reproducible from the env seed
    ...
    return all_coords[rng.integers(len(all_coords))]


def spawn_plants(grid_dims, how_many, used_coordinates, rng):
    new_plants = []
    for x in range(how_many):
        new_plant = zeros(2, dtype=uint8)
        new_pos = place_entity_in_unoccupied_cell(
            grid_dims=grid_dims, used_coordinates=new_plants + used_coordinates, rng=rng
        )
...
def respawn_plants(plants, tagged_plants, grid_dims, used_coordinates, rng):
    for tagged_plant in tagged_plants:
        new_plant = zeros(2, dtype=uint8)
        new_pos = place_entity_in_unoccupied_cell(
            grid_dims=grid_dims, used_coordinates=plants + used_coordinates, rng=rng
        )
        new_plant[0], new_plant[1] = new_pos
